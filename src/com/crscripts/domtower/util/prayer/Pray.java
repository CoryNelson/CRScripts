...
public class Pray {
	...
	public static boolean prayerActive(int id) {
		PrayerBook[] activeP = Pray.getActive();
		for (PrayerBook active : activeP)
			if (active.getId() == id)
				return true;
		return false;
	}

	public static void toggle(int prayerId) {
		if (!Tabs.PRAYER.isOpen())
			Tabs.PRAYER.open();
		boolean before = prayerActive(prayerId);
		if (Widgets.get(271, 7).getChild(prayerId).click(true)) {
			int time = 0;
			while (prayerActive(prayerId) == before && time <= 5000) {
				Time.sleep(50);
				time += 50;
			}
			Time.sleep(200, 500);
